...
//  char strIn [35] = "/sys/devices/ocp.3/helper.12/AIN4"; // Changed from helper.15 to helper.12
  char strIn [50] = "/sys/bus/iio/devices/iio:device0/in_voltage4_raw";
  ...
  if (argc != 2) {
    ...
    printf("The note starts at 15 = Eb and increases by one half step\n");
    printf("For every integer increase of 1, to a maximum of 22 = Bb.\n\n");
  ...
  while (aIn < THRESHOLD) {
    poll(&strFile, 1, -1);
    aIn = getAnalog(STRING4, strFile.fd);
  ...
  for (int i = 0; i < NUM_CYCLES; i++) {
    ...
    poll(&strFile, 1, -1);
    waveform[i] = getAnalog(STRING4, strFile.fd);
    lseek(strHandle, 0, SEEK_SET);

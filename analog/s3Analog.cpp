...
const int LOW_LIMIT = NUM_CYCLES / 17;
const int PEAK_LIMIT = NUM_CYCLES / 10;
...
//  char waveOut [35] = "/root/code/waveform.txt";
  ...
  if (argc != 2) {
    ...
    printf("The note starts at 10 = Bb and increases by one half step\n");
    printf("For every integer increase of 1, to a maximum of 17 = F.\n\n");
  ...
  while (aIn < THRESHOLD) {
    poll(&strFile, 1, -1);
    aIn = getAnalog(STRING3, strFile.fd);
